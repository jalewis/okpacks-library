...
A pack is the domain layer OKEngine builds a vault from: a `schema.yaml` (its **domain**
types + rules), a `CLAUDE.md` persona, feed sources, and crons. The universal OKF core
(types like `source` / `concept` / `finding`; the `entities` / `sources` / … namespaces)
is **engine-owned and inherited** — a pack declares only its domain layer and `extends:`
the core for any extra fields (see the engine's `docs/core-types-and-extensions.md`).
**This library holds pack DEFINITIONS only — with empty `wiki/` scaffolds.** You take a definition, point
...
| **[`okpack-sec`](packs/okpack-sec)** | **security / threat-intel BUNDLE** (STIX-aligned) — composes the six CTI packs below | bundle |
...
The security packs demonstrate **composition** (OKEngine v0.10.3): each owns a disjoint slice of
types, and the library enforces globally-unique type ownership so any set composes into one vault.
**`okpack-sec` is a bundle** (`kind: bundle`) — it owns nothing and declares a recipe that composes
the six CTI packs. `framework pull okpack-sec` fetches the host (`okpack-threat-actors`) as the base
...
[`packs/okpack-sec/README.md`](packs/okpack-sec/README.md).
