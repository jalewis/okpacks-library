...
CROSS-PACK invariants those validators can't see, in one place:
  ...
  - **pack README completeness** (every catalog pack is documented, with no stale entries);
...
def check_catalog_consistency(catalog: dict) -> None:
    ...
    for p in catalog.get("packs", []):
        ...
        if not py.exists():
            ...
            if str(p.get("version", "")) != str(meta.get("version", "")):
                fail(f"catalog: {name}: catalog version {p.get('version')!r} != "
                     f"pack.yaml version {meta.get('version')!r}")
...
def check_pack_readme_catalog(catalog: dict) -> None:
    """Both human-facing pack tables must match canonical catalog metadata exactly."""
    try:
        drifted = synchronize(ROOT, check=True)
    except (OSError, ValueError) as error:
        fail(f"pack catalog documentation cannot be generated: {error}")
        return
    for path in drifted:
        fail(f"{path.relative_to(ROOT)} pack catalog is stale — "
             "run python3 scripts/generate_pack_catalog.py")
...
def main() -> int:
    ...
    check_pack_readme_catalog(catalog)
    check_dev_deps()
