...
# engine/src carries the `okengine` PACKAGE. scripts/cron modules import from it
# (corpus_audit -> okengine.actor_identity), so omitting it makes those modules
# unimportable here even though they import fine inside a deployment, where the engine
# is pip-installed. okpacks-library#89.
sys.path[:0] = [str(data / "scripts"), str(engine / "scripts"),
                str(engine / "scripts" / "cron"), str(engine / "src")]
