#!/usr/bin/env python3
"""okpack-threat-landscape feed pull — fetch the threat-landscape OPML into raw/threat-landscape/ for the ingest lane.
...
Env: WIKI_PATH (default /opt/vault), OKPACK_THREAT_LANDSCAPE_FEEDS (default the deployed
...
raise SystemExit(feed_fetch.main([
    "--opml", os.environ.get("OKPACK_THREAT_LANDSCAPE_FEEDS", "/opt/data/config/feeds.opml"),
    "--out-dir", f"{WIKI}/raw/threat-landscape",
    "--state", "/opt/data/scripts/okpack-threat-landscape-feed-state.json",
    "--source-tag", "threat-landscape",
]))
