...
def test_checker_catches_bad_pages():
    schema = _schema()
    assert check_page({"type": "source", "source_kind": "bogus", "published": "2026-01-01"}, schema)  # bad closed enum
    assert check_page({"type": "source"}, schema)                          # source missing `published`
    assert not check_page({"type": "source", "source_kind": "annual-report",
                           "published": "2026-01-01"}, schema)             # valid core source
    assert check_page({"type": "metric", "metric_direction": "sideways"}, schema)  # bad closed enum (by_type metric)
    # extensible enum: an unknown metric_unit is allowed (warn-not-fail) -> no error
    assert not check_page({"type": "metric", "metric_direction": "up", "metric_unit": "furlongs"}, schema)
...
if __name__ == "__main__":
    print("== okpack-threat-landscape page conformance ==")
    n = _run()
