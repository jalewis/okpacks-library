# okpack-threat-landscape — Threat-Landscape / Annual-Report Intelligence: persona & curation rules

This is the guidance the engine's cron agents read at runtime (`$WIKI_PATH/CLAUDE.md`).
It is the *domain voice + ingest/curation workflow* for **okpack-threat-landscape** — distinct from the
...
Maintain a compounding vault that mines the **annual security-report firehose** (DBIR, M-Trends,
threat-landscape + industry surveys) into landscape signal: which **vendors** publish what, which
**themes** are rising or fading year over year, and which **metrics** (breach cost, dwell time,
ransomware payment) the field converges or disagrees on — so a strategist can read the landscape from
the vault instead of a hundred PDFs. The complement to actor-tracking and vuln-tracking; composes with both.
...
- **Audience:** a CISO / security strategist / threat-landscape analyst. Assume expertise — skip 101
...
entity, concept, trend, prediction, finding, or briefing. Incomplete extraction is deferred or failed.
...
2. **Score every source.**
   Score by `source_kind` (annual-report / survey / threat-report) and the publishing vendor's rigor
   (methodology, sample size, first-party telemetry vs. survey). Report vendors are self-interested —
   note it. When vendors DISAGREE on a metric, keep the range; don't average it away.
...
4. **Extract entities downstream from accepted sources** — every changed entity cites a
   ...
   **`entities/<first-letter-of-slug>/<slug>`** (the engine shards by the slug's FIRST
   ...
   The domain types (schema.yaml `types`): **publisher** (a report publisher — how many reports, which
   themes, which years) and **metric** (a statistic tracked across reports/years, with `metric_direction`
   + `metric_unit`). Reports themselves are `source` pages stamped with an inferred `report_theme`.
...
6. **Findings are HUMAN-AUTHORED.** You may *surface* candidate findings in your run
   ...
   (No human-only namespace gate here; the analysis lanes write publisher/trend pages directly.)
...
- **Standards:** no single external ID scheme; the anchor is the `report_theme` taxonomy (ransomware,
  ai-security, cloud, supply-chain, …) + the publishing vendor and year.
