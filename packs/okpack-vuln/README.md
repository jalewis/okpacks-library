# okpack-vuln — Vulnerability / Exploited-CVE Tracking

The **exploited-vulnerability layer** of a CTI vault: one canonical page per CVE, seeded from CISA's
Known Exploited Vulnerabilities (KEV) catalog — the high-signal subset that is *actually being
exploited in the wild*, not the full NVD firehose. Built to compose: it owns `cve`, so an adversary
pack's `[[cve/...]]` links resolve to real vulnerability pages, and each CVE gains automatic
backlinks to every actor and malware that exploits it.

Pinned to engine **okengine v0.10.8** (see `engine.version`).

## What it builds

- **Owns** `cve` (namespace `cves`).
- **Seeded from** the CISA KEV catalog (exploited-in-the-wild CVEs).
- **Key output** — one canonical page per CVE, backlinked from every actor/malware that exploits it.
- **Composes** — the exploited-vuln layer of the CTI stack; resolves the `[[cve/...]]` links other packs emit.
- **OSV precision** — enriches existing CVEs with package/PURL, affected/fixed version,
  alias, withdrawal, and advisory-revision evidence without creating duplicate pages.
- **Action queue** — separates Act now, Investigate, Monitor, and Deprioritize using
  explicit KEV, EPSS freshness, applicability, exposure, criticality, remediation,
  and corroboration factors. Missing inputs remain visibly unknown.

OSV raw revisions are content-addressed under runtime data (`.hermes-data/osv-archive`
or `.okengine/osv-archive`), not published under `wiki/`. The current normalized
projection is merge-stamped on each CVE. Multiple CVE aliases are flagged with
`osv_alias_ambiguity: true`; they never merge canonical pages automatically.

`applicability`, `asset_exposure`, and `business_criticality` are optional composition
fields for VEX or an organization-specific exposure overlay. A vendor publishing a fixed
version only establishes that remediation exists. Only explicit `applicability: fixed`
means the assessed asset is fixed.
...
publishes a host port, offset by **+210** to avoid colliding with other packs
(okengine#138).

| Service | Container | Host port → internal | Role |
|---------|-----------|----------------------|------|
| `gateway` | `okpack-vuln-gateway` | none (bridge; reaches `okengine-mcp` by service name) | Hermes agent runtime + delivery |
| `okengine-reader` | `okpack-vuln-reader` | `9410 → 9200` | search/read index over the vault (`:ro`) |
| `okengine-mcp` | `okpack-vuln-mcp` | none by default (`8940 → 8730` only if exposed to external agents) | enforced MCP write/query surface (`WIKI_PATH=/opt/vault`) |
