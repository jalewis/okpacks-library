...
counting how many source/briefing documents mention each CVE ID in their prose (a raw `CVE-YYYY-NNNN`
appearance, no wikilink needed — that's how reporting actually cites them). Does two things:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for sub in _SCAN_DIRS:
        ...
        for p in base.rglob("*.md"):
            if p.stem.startswith(("INDEX", "_")):
                continue
            fm, body = _split(p)
            cves = {m.upper() for m in _CVE_RE.findall(body)}
            if not cves:
                continue
            ndocs += 1
    ...
    lines = [f"# Top {len(top)} CVEs by reporting", "",
             f"> Ranked by how many source/briefing documents mention the CVE ID in prose "
