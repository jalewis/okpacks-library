...
def _cwes(v: dict) -> list[str]:
    """Normalized CWE ids from a KEV entry's `cwes` list (present since mid-2024;
    older feed snapshots simply lack it — returns [])."""
    out = []
...
def _load_existing_fm(path: Path) -> dict:
    ...
    try:
        ...
        return {}  # page moved/deleted by a concurrent lane between exists() and read
...
def main(argv: list[str] | None = None) -> int:
    ...
    for v in cat.get("vulnerabilities", []):
        ...
        fm = {
            ...
            "cwe": _cwes(v) or None,   # weakness taxonomy — feeds the CWE trend rollup (epss_import)
            "sources": ["CISA KEV"],
            "url": f"https://nvd.nist.gov/vuln/detail/{cid}",
        }
        # MERGE: preserve fields other packs/lanes set (e.g. curated cvss_score, exploited_by).
        # find_page locates the CVE wherever it lives (canonical shard OR a stale flat copy) so we
        # merge in place instead of re-creating a flat duplicate every cycle (okengine#54).
        prev_path = okf_migrate.find_page(root, "cves", cid)
        prev = _load_existing_fm(prev_path) if prev_path else {}
        ...
        if args.dry_run:
            written += 1
            continue
        # write to the canonical shard the reshelve drain would choose, and drop any stale copy at a
        # different path so repeated runs converge instead of doubling (okengine#54).
