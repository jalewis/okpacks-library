#!/usr/bin/env python3
"""okpack-vuln feed pull — fetch the vuln OPML into raw/vuln/ for the ingest lane.
...
Env: WIKI_PATH (default /opt/vault), OKPACK_VULN_FEEDS (default the deployed
...
raise SystemExit(feed_fetch.main([
    "--opml", os.environ.get("OKPACK_VULN_FEEDS", "/opt/data/config/feeds.opml"),
    "--out-dir", f"{WIKI}/raw/vuln",
    "--state", "/opt/data/scripts/okpack-vuln-feed-state.json",
    "--source-tag", "vuln",
]))
