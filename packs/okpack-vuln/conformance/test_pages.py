...
def test_checker_catches_bad_pages():
    ...
    assert not check_page({"type": "source", "published": "2026-01-01"}, schema)   # valid core source
    assert check_page({"type": "cve", "exploitation_status": "bogus"}, schema)     # bad closed enum
    assert not check_page({"type": "cve", "exploitation_status": "actively-exploited",
                           "severity": "high"}, schema)                            # valid
    # extensible enum: an unknown severity is allowed (warn-not-fail) -> no error
    assert not check_page({"type": "cve", "severity": "catastrophic"}, schema)
...
if __name__ == "__main__":
    print("== okpack-vuln page conformance ==")
    n = _run()
