# okpack-vuln — Vulnerability / Exploited-CVE Tracking: persona & curation rules

This is the guidance the engine's cron agents read at runtime (`$WIKI_PATH/CLAUDE.md`).
It is the *domain voice + ingest/curation workflow* for **okpack-vuln** — distinct from the
...
Maintain a compounding vault of **actively-exploited vulnerabilities**: ingest CISA's Known
Exploited Vulnerabilities (KEV) catalog (plus vendor/CERT advisories) and compile them into one
canonical `cve` page each — so a defender can answer *which CVEs are being exploited right now, by
whom, and how urgently to patch*, and so an adversary pack's `[[cve/…]]` links resolve here when
composed.
...
- **Audience:** a vulnerability manager / SOC analyst / detection engineer. Assume expertise — skip 101
...
2. **Score every source.**
   CISA KEV is authoritative for the *exploited* signal (`exploitation_status: actively-exploited`);
   NVD/vendor advisories for CVSS `severity` + technical detail. Flag `known_ransomware` where CISA does.
...
4. **Extract entities downstream from accepted sources** — every changed entity cites a
   ...
   **`entities/<first-letter-of-slug>/<slug>`** (the engine shards by the slug's FIRST
   ...
   The domain type (schema.yaml `types`): **cve** — one canonical page per CVE keyed by `cve_id`
   (id_authority `cve`, so it converges with any pack referencing the same CVE). Fields: `severity`,
   `exploitation_status`, `vendor`, `product`, `date_added`, `due_date`, `kev`, `known_ransomware`.
...
6. **Findings are HUMAN-AUTHORED.** You may *surface* candidate findings in your run
   ...
   (No human-only namespace gate here; the KEV importer writes cve pages directly.)
...
- **Standards:** CVE (the identifier), CVSS (severity), the CISA KEV catalog (exploited-in-the-wild),
  CWE (weakness class).
