#!/usr/bin/env python3
"""okpack-detections feed pull — fetch the detections OPML into raw/detections/ for the ingest lane.
...
Env: WIKI_PATH (default /opt/vault), OKPACK_DETECTIONS_FEEDS (default the deployed
...
raise SystemExit(feed_fetch.main([
    "--opml", os.environ.get("OKPACK_DETECTIONS_FEEDS", "/opt/data/config/feeds.opml"),
    "--out-dir", f"{WIKI}/raw/detections",
    "--state", "/opt/data/scripts/okpack-detections-feed-state.json",
    "--source-tag", "detections",
]))
