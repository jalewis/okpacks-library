# okpack-detections — Detections & Mitigations

The **defensive layer** of a CTI vault: a cross-linked catalog of `detection` rules (Sigma/YARA/EQL
— what you watch for) and `course-of-action` mitigations (MITRE ATT&CK M#### — what you do about a
threat). Every detection links to the ATT&CK technique(s) it covers, so a security engineer can ask
the vault *which techniques do we actually detect, and where are the gaps?*

Pinned to engine **okengine v0.10.8** (see `engine.version`).

## What it builds

- **Owns** `detection`, `course-of-action` (namespace `detections`).
- **Seeded from** the open SigmaHQ ruleset and MITRE ATT&CK mitigations.
- **Key output** — a technique-coverage map: each detection cross-links the `[[technique]]` it covers.
- **Composes** — the defensive layer of the CTI stack; techniques resolve into a composed actor pack
  ([`okpack-threat-actors`](../okpack-threat-actors)).
...
publishes a host port, offset by **+250** to avoid colliding with other packs
(okengine#138).

| Service | Container | Host port → internal | Role |
|---------|-----------|----------------------|------|
| `gateway` | `okpack-detections-gateway` | none (bridge; reaches `okengine-mcp` by service name) | Hermes agent runtime + delivery |
| `okengine-reader` | `okpack-detections-reader` | `9450 → 9200` | search/read index over the vault (`:ro`) |
| `okengine-mcp` | `okpack-detections-mcp` | none by default (`8980 → 8730` only if exposed to external agents) | enforced MCP write/query surface (`WIKI_PATH=/opt/vault`) |
