...
def test_checker_catches_bad_pages():
    ...
    assert not check_page({"type": "source", "source_kind": "ruleset",
                           "published": "2026-01-01"}, schema)             # a valid source passes
    assert check_page({"type": "detection", "detection_format": "nope"}, schema)   # bad detection_format
    assert check_page({"type": "detection", "rule_level": "nope"}, schema)         # bad rule_level
    assert not check_page({"type": "detection", "detection_format": "sigma", "rule_level": "high"}, schema)
...
if __name__ == "__main__":
    print("== okpack-detections page conformance ==")
    n = _run()
