...
def test_checker_catches_bad_pages():
    ...
    assert not check_page({"type": "source", "source_kind": "breach-db",
                           "published": "2026-01-01"}, schema)             # a valid source passes
    assert check_page({"type": "incident", "incident_type": "nope"}, schema)   # bad incident_type
    assert check_page({"type": "identity", "identity_class": "nope"}, schema)  # bad identity_class
    assert not check_page({"type": "incident", "incident_type": "hacking"}, schema)
...
if __name__ == "__main__":
    print("== okpack-incidents page conformance ==")
    n = _run()
