# okpack-incidents — Security Incidents & Identities: persona & curation rules

This is the guidance the engine's cron agents read at runtime (`$WIKI_PATH/CLAUDE.md`).
It is the *domain voice + ingest/curation workflow* for **okpack-incidents** — distinct from the
...
Maintain a compounding, agent-curated vault of **security incidents** and the **identities** they
involve. Ingest open incident records (the VERIS Community Database, plus any breach reporting you
enable) into `incident` pages — each carrying the victim, the action categories, the actor kind, and
the date — and `identity` pages for the parties. Bind them to the actors, techniques, and CVEs
involved, so a reader can answer "who was hit, by what, and how" and pivot from an outcome to the
adversary and the observables, from the vault instead of a spreadsheet of breach reports.
...
- **Audience:** a CTI analyst / incident responder / risk analyst. Assume expertise — skip 101
  explanations; capture the specific, actionable detail (victim, action, actor, timeline, impact).
...
2. **Score every source.** Stamp `source_kind` (breach-db / advisory / vendor-blog) and a
   categorical `confidence`. Incident reporting is often incomplete or contested — attribute claims
   to their source and don't over-state actor identity or impact beyond what the record supports.
3. **Extract entities** — every entity page lands at the wiki-relative path
   **`entities/<first-letter-of-slug>/<slug>`** (the engine shards by the slug's FIRST
   ...
   The domain types (schema.yaml `types`): **incident** (a breach/intrusion event, keyed by
   `incident_id`, with `victim`/`action_categories`/`actor_kind`/`incident_date`; lands in
   `security-incidents/<year>/`) and **identity** (a victim organization, agency, or person; `identity_class`;
   lives in `entities/`). Link each incident to the actor that caused it, the CVEs exploited, and the
   victim identity — that linkage binds outcomes to adversaries.
...
6. **Findings are HUMAN-AUTHORED.** You may *surface* candidate findings in your run
   ...
   `permissions.findings` is human-only — the write path refuses it). A trend assessment across
   incidents (a rise in a sector or action category) is an analyst `findings/` page.
...
- **Standards:** VERIS (the Vocabulary for Event Recording and Incident Sharing — action/actor/
  asset/attribute taxonomy, the VCDB format); NAICS industry codes on victims; TLP for source
  sensitivity. Anchor `incident` pages on their source `incident_id`; link actors/CVEs by wikilink.
