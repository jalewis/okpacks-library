...
The **event layer** of a CTI vault: a ledger of security `incident` events (breaches, intrusions,
data-loss) and the `identity` parties they involve (victim organizations, agencies, persons), seeded
from the open VERIS Community Database. It binds actors, techniques, and indicators to real-world
outcomes — so a reader can ask *who was hit, by what, and how,* and pivot from an incident to the
adversary behind it.
...
- **Owns** `incident`, `identity` (namespace `security-incidents`).
- **Seeded from** the open VERIS Community Database (VCDB).
- **Key output** — incident pages that link out to the actors, techniques, and indicators involved.
- **Composes** — the event layer of the CTI stack; pivots into the actor, indicator, and detection packs.
