#!/usr/bin/env python3
"""okpack-indicators feed pull — fetch the indicators OPML into raw/indicators/ for the ingest lane.
...
Env: WIKI_PATH (default /opt/vault), OKPACK_INDICATORS_FEEDS (default the deployed
...
raise SystemExit(feed_fetch.main([
    "--opml", os.environ.get("OKPACK_INDICATORS_FEEDS", "/opt/data/config/feeds.opml"),
    "--out-dir", f"{WIKI}/raw/indicators",
    "--state", "/opt/data/scripts/okpack-indicators-feed-state.json",
    "--source-tag", "indicators",
]))
