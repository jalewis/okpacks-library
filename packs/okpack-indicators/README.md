# okpack-indicators — Indicators & Infrastructure (IOCs)

The **observable layer** of a CTI vault: a catalog of atomic indicators of compromise (malicious
URLs, domains, IPs, file hashes) and the adversary `infrastructure` behind them, seeded from open
feeds (URLhaus / abuse.ch). Actor, malware, and campaign pages point *at* these observables — so a
reader can pivot from *who* to the concrete artifacts, and every indicator carries automatic
backlinks to whatever references it.

Pinned to engine **okengine v0.10.8** (see `engine.version`).

## What it builds

- **Owns** `indicator`, `infrastructure` (namespace `indicators`).
- **Seeded from** open IOC feeds — URLhaus / abuse.ch.
- **Key output** — indicator pages with automatic backlinks from the actor/malware/campaign pages that cite them.
- **Composes** — the observable layer of the CTI stack; the concrete artifacts the actor pack points at.
...
publishes a host port, offset by **+240** to avoid colliding with other packs
(okengine#138).

| Service | Container | Host port → internal | Role |
|---------|-----------|----------------------|------|
| `gateway` | `okpack-indicators-gateway` | none (bridge; reaches `okengine-mcp` by service name) | Hermes agent runtime + delivery |
| `okengine-reader` | `okpack-indicators-reader` | `9440 → 9200` | search/read index over the vault (`:ro`) |
| `okengine-mcp` | `okpack-indicators-mcp` | none by default (`8970 → 8730` only if exposed to external agents) | enforced MCP write/query surface (`WIKI_PATH=/opt/vault`) |
