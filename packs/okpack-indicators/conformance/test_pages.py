...
def test_checker_catches_bad_pages():
    ...
    assert not check_page({"type": "source", "source_kind": "feed",
                           "published": "2026-01-01"}, schema)             # a valid source passes
    assert check_page({"type": "indicator", "indicator_type": "not-a-type"}, schema)   # bad indicator_type
    assert check_page({"type": "infrastructure", "infra_type": "not-a-type"}, schema)  # bad infra_type
    assert not check_page({"type": "indicator", "indicator_type": "url"}, schema)      # valid indicator
...
if __name__ == "__main__":
    print("== okpack-indicators page conformance ==")
    n = _run()
