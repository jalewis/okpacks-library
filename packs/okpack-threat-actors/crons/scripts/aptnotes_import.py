...
# Structured actor IDs sit BELOW the length floor but are the least ambiguous names in the corpus,
# and they are how threat-intel reports title themselves. The floor exists to stop short free-text
# aliases false-matching ("AMD" resolves to an actor here), but it was also excluding the entire
# APT<nn>/FIN<n>/UNC<nn> convention: APT1 is 4 characters, APT28 and FIN7 are 5.
#
# Measured on a live vault: 1,808 historical sources, ~1.3% linked to any actor, against 91.3% for
# the connector-ingested feed. Mandiant's "APT1: Exposing One of China's Cyber Espionage Units" --
# stored, graded A/1 -- linked to nothing, so APT1's country assessment read `single-source` on the
# strength of an aggregator re-publishing MITRE's paragraph.
#
# Length was the wrong instrument for ambiguity: `\bAPT1\b` cannot match APT12 -- the word boundary
# already prevents the collision the floor was guarding against. Admit structured IDs, keep the
# floor for free-text. G-numbers require MITRE's 4-digit form so "G20" cannot match.
STRUCTURED_ID = re.compile(r"(?:APT|FIN|UNC|TA)[ -]?\d{1,4}|G\d{4}", re.I)
...
def _actor_matcher(idx: dict):
    ...
    for alias, rel in idx.items():
        if len(alias) >= MIN_ALIAS or STRUCTURED_ID.fullmatch(alias):
