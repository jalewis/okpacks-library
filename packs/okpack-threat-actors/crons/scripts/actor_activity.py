...
carry `mentions_actors` + a `published` date. Does two things:
  1. STAMPS `recent_reports` + `total_mentions` onto each actor page (the report-mention counts), so
     the cockpit can show each actor's report pedigree. `recent_reports` is clamped to `total_mentions`.
     NEWS recency (news_last_seen) is a SEPARATE lane — actor_news_activity.py — because mentions_actors
     is written only by the report importers, so it can never reflect the news firehose.
...
def main(argv: list[str] | None = None) -> int:
    ...
    for slug, c in counts.items():
        ...
        # `recent` is a subset of `total` by construction (it only increments inside the total
        # branch). Clamp anyway so a value that drifted above total from another writer — an agent
        # once hand-set `recent_reports` to a list of source paths (len) — is corrected on every run.
        recent = min(c["recent"], c["total"])
        ...
        atier = "hot" if recent >= 3 else "warm" if recent >= 1 else "cold"
        # last_updated: the cockpit watchlist's moved_field — write_page merges but does not bump the
        # envelope, and imported actor pages carry no `updated` at all, so without this stamp the
        # "Recently moved / Gone quiet" sections are permanently empty.
        stamp = {"type": "actor", "recent_reports": recent, "total_mentions": c["total"],
                 "activity_tier": atier,
                 "last_updated": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")}
        try:
            write_page(wiki, rel, stamp, body, dry_run=args.dry_run)
            stamped += 1
        except OSError as e:
            print(f"WARN: stamp {rel}: {e}", file=sys.stderr)

    # 2) the top-N dashboard — report pedigree, ranked by report-mention volume (recent, then total).
    ranked = sorted(counts.items(), key=lambda kv: (kv[1]["recent"], kv[1]["total"]), reverse=True)
    top = ranked[:args.top_n]
    lines = [f"# Top {len(top)} threat actors by report activity", "",
             f"> Ranked by mentions in reporting published in the last {args.recent_days // 365}y "
             f"(since {cutoff}), across {nsrc} source pages. Report pedigree — for NEWS recency see the "
             "cockpit **Recently active** board (actor_news_activity). Regenerated `no_agent`.", "",
             "| # | Actor | Recent reports | Total mentions |", "|---:|---|---:|---:|"]
    for i, (slug, c) in enumerate(top, 1):
        name = idx.get(slug, (slug, ""))[0]
        lines.append(f"| {i} | [[{slug}|{name}]] | {min(c['recent'], c['total'])} | {c['total']} |")
