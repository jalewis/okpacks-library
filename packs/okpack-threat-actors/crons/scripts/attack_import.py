...
MITRE_AUTHORITY_POLICY = {
    "id": "mitre-attack-stix-v1",
    "authority": "MITRE ATT&CK",
    "eligible_types": ["actor", "campaign", "malware", "tool", "technique", "concept"],
    "source_names": ["MITRE ATT&CK"],
    "url_hosts": ["attack.mitre.org"],
    "url_path_pattern": r"/(groups/G\d{4}|campaigns/C\d{4}|software/S\d{4}|techniques/T\d{4}(?:/\d{3})?|mitigations/M\d{4})/?",
    "id_field": "attack_id",
    "id_pattern": r"(?:G|C|S|T|M)\d{4}(?:\.\d{3})?",
    "verified_fields": ["title", "aliases", "attack_id", "description", "platforms", "techniques", "software"],
    "required_values": {"authority_import": "mitre-attack-stix"},
}


def _reviewed_at(value: str | None) -> str:
    """Normalize ATT&CK's object modification time to the engine's UTC-second contract."""
    if not value:
...
def build_records(bundles: list[dict]) -> tuple[dict, dict]:
    ...
    def _register(stix_id, ptype, key, path, name, fm, desc, modified):
        records[stix_id] = {"ptype": ptype, "key": key, "path": path, "name": name,
                            "fm": fm, "desc": desc, "links": {},
                            "reviewed_at": _reviewed_at(modified)}
    ...
    for b in bundles:
        for o in b.get("objects", []):
            ...
            if t in _ENTITY_KIND:
                ...
                _register(o["id"], ptype, key, f"entities/{key}.md", name, fm, desc, o.get("modified"))
                ...
                fm = {"type": "technique", "id": aid or key, "title": name, "attack_id": aid,
                      ...
                      "sources": ["MITRE ATT&CK"], "url": _url(o),
                      "authority_import": "mitre-attack-stix"}
                _register(o["id"], "technique", key, f"techniques/{key}.md", name, fm, desc, o.get("modified"))
                ...
                fm = {"type": "concept", "id": aid or key, "title": name, "concept_kind": "mitigation",
                      ...
                      "authority_import": "mitre-attack-stix"}
                _register(o["id"], "concept", key, f"concepts/{key}.md", name, fm, desc, o.get("modified"))
...
def render_body(rec: dict) -> str:
    ...
    lines.append("> Directly imported from MITRE ATT&CK. The ATT&CK record fields above are accepted "
                 "under the MITRE authority policy; claims derived beyond that record require a separate assessment.")
    return "\n".join(lines)
    ...
    for rec in recs:
        ...
        try:
            action = write_page(vault, rec["path"], rec["fm"], render_body(rec), dry_run=args.dry_run,
                                authority_policy=MITRE_AUTHORITY_POLICY,
