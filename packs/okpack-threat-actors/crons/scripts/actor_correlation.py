    ...
    for rel, a in actors.items():
        ...
        for orel, b in actors.items():
            ...
            if score >= args.min_score:
                ...
                scored.append((score, b["name"], [f.split(":", 1)[1] for f in top]))
        if not scored:
            continue
        scored.sort(reverse=True)
        related = [{"actor": name, "score": round(s, 2), "shared": ev}
                   for s, name, ev in scored[:args.top_k]]
        ...
        for r in related:
            body_lines.append(f"- **{r['actor']}** (score {r['score']}) — shared: {', '.join(r['shared'])}")
        try:
