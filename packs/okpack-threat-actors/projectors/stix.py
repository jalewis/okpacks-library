"""Project okf-cti pages → STIX 2.1.
...
conformance fixtures possible. Fields STIX 2.1 has no native home for ride an `x_okfcti_*` custom
...
# Fixed namespace for okf-cti deterministic id derivation (not a STIX-mandated value).
NS = uuid.UUID("6f9b9af4-0000-5e00-8a01-a1b2c3d4e5f6")
SPEC_VERSION = "2.1"

# okf-cti canonical type → default STIX SDO type. `source`→`report`; types with no SDO
# (software, detection, concept, finding, prediction, dashboard) get the generic x_okfcti path.
SDO_TYPE = {
...
REL = {
    ...
    "detected-by": ("x_okfcti_detects", True), "owned-by": ("owns", True), "hosted-by": ("hosts", True),
    "affected-by": ("x_okfcti_affects", True), "attributed-campaigns": ("attributed-to", True),
    # okf-native (no STIX SRO) → custom relationship_type string (valid: it's open-vocab):
    "detects": ("x_okfcti_detects", False), "affects": ("x_okfcti_affects", False),
}
...
def page_stix_id(page: dict, slug: str) -> str:
    ...
    stix_type = stix_hint.get("type") or SDO_TYPE.get(okf_type) or f"x-okfcti-{okf_type}"
...
def project_page(page: dict, slug: str, resolver: dict | None = None) -> dict:
    """Project one okf-cti page (its frontmatter dict) to a STIX 2.1 bundle. `resolver` (from
    ...
    if stix_type is None:
        # No STIX SDO for this okf-cti type: emit a custom object carrying everything as loss.
        stix_type = f"x-okfcti-{okf_type}"
    ...
    # --- documented loss: any remaining non-envelope field → x_okfcti_<field> ---
    ...
    for pred, targets in (page.get("rels") or {}).items():
        ...
        rtype, reverse = REL.get(pred, (f"x_okfcti_{pred}", False))
