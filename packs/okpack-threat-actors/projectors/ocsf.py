"""Project okf-sec event-layer pages → OCSF (Detection Finding, class_uid 2004).

The two-altitude model (spec §1.3): entity pages → STIX 2.1 (`projectors/stix.py`);
event/observation records → OCSF. okf-sec's OCSF-facing types are `finding` (an analyst
...
STIX `x_okfsec_*`). Entity types return None (they are STIX, not OCSF).
...
OCSF_VERSION = "1.3.0"
PRODUCT = {"name": "okf-sec", "vendor_name": "okpack-sec"}
...
# okf-sec `severity` → OCSF severity_id (0 Unknown · 1 Informational · 2 Low · 3 Medium · 4 High · 5 Critical · 6 Fatal)
...
def project_page(page: dict, slug: str):
    """Project one okf-sec page to an OCSF Detection Finding. None for non-event types."""
