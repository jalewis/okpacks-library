...
- **Owns** `actor`, `campaign`, `malware`, `tool`, `technique` — the adversary graph (namespace
  `techniques` plus the shared entity namespaces).
- **Seeded from** MITRE ATT&CK, the MISP threat-actor galaxy, and public CERT/vendor reporting.
- **Key outputs** — one canonical page per actor (every vendor alias reconciled), actor↔actor
  correlation by shared tradecraft, and technique-coverage views.
- **Composes** — the anchor of the CTI stack; other packs' `[[actor]]` / `[[technique]]` links resolve here.
...
### Exporting public evidence for private analysis

`actor_evidence_export.py` creates an allowlisted evidence snapshot for a separate private analysis
pack. It exports structured actor identity, reported country nexus, attribution confidence,
behaviors, normalized source metadata, and an explicit derived country-nexus claim. Claim records
distinguish structured-field renderings or corpus attribution notes from verbatim source text and
leave source independence and lineage `not-assessed`. Snapshot contract v2 separates exact,
claim-specific citations (`source_refs`) from entity provenance or publisher-level context
(`context_source_refs`). An actor's exact MITRE ATT&CK group URL is promoted to a claim-specific
guidepost; generic publisher and dataset roots are never represented as claim support. It never
exports page bodies, findings, prompts, configuration, or unsourced actors.

```bash
python3 crons/scripts/actor_evidence_export.py /path/to/new-snapshot --vault /path/to/vault
```

The output contains `manifest.json`, `actors.ndjson`, and an export report listing records excluded
for missing provenance. The destination must be new; the exporter never overwrites an existing
snapshot.
