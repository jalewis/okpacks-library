...
All notable changes to the okf-cti spec + reference implementation. Changes are classified per
spec §12.2 — **Breaking** / **Additive** / **Editorial**. Format loosely follows
[Keep a Changelog]. okf-cti uses `MAJOR.MINOR`; **pre-1.0, a MINOR bump may break.**

[Keep a Changelog]: https://keepachangelog.com/

## 0.3.1 — **Editorial**: renamed `okpack-sec` → `okpack-cti` (+ the `okf-sec` spec identity → `okf-cti`)

The bundle was renamed to reflect that it composes the *cyber-threat-intelligence* packs
specifically (adversaries, exploited CVEs, indicators, detections, incidents, threat-landscape);
other security packs (e.g. `okpack-vendor-risk`) sit outside it. No content or recipe change —
`framework pull okpack-cti` replaces `framework pull okpack-sec`, and the old name no longer
resolves. The `okf-sec` profile/spec identity was renamed to `okf-cti` alongside it, including the
STIX/OCSF custom namespaces the projectors emit (`x_okfsec_*` → `x_okfcti_*`, `x-okfsec-*` →
`x-okfcti-*`) and the OCSF product name — a projected-output change, but pre-1.0 and pinned by no
conformance golden. The historical entries below are preserved under the names they shipped as
(`okpack-sec` / `okf-sec`).
...
### Added (additive — §12.2)
- **OKF-CTI-SPEC §13 Multi-source entity resolution (MDM)** — documents the multi-source
...
### Added (additive — §12.2)
- **`incident` type** (OKF-CTI-SPEC §8, schema.yaml, CLAUDE.md) — a first-class security
...
- **`threat-actor` vs `intrusion-set` typing rule sharpened** (OKF-CTI-SPEC §5, schema comments,
...
- Spec header: lineage note crediting the Google Open Knowledge Format (OKF) as the base format
  (formalizing Karpathy's "LLM wiki" pattern); okf-cti is its security profile.
...
- Versioning: `schema.yaml` `okf_cti_version` pins the spec version a pack implements.
