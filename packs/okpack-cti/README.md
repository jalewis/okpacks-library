# okpack-cti — the security bundle (STIX-aligned CTI pack family)

> **okpack-cti is now a pack _bundle_ (okengine#181), not a monolith.** It owns no types and
...
one vault — easier to reuse, extend, and reason about. `okpack-cti` is the convenience installer
that recomposes them.

## What it installs

`framework pull okpack-cti` resolves the recipe in [`pack.yaml`](pack.yaml): it pulls the **host**
...
```bash
framework pull okpack-cti          # resolves the recipe: host + install-domain each compose pack
```
...
- **One-command parity** — the bundle preserves "pull okpack-cti → get the full security vault".
