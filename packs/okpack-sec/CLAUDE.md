...
- **Working directory / vault root:** `/opt/vault`. The OKF page tree lives under
  **`/opt/vault/wiki/`** (`wiki/sources/`, `wiki/entities/`, `wiki/briefings/`, …);
  ...
  `/opt/vault/schema.yaml`). Relative `wiki/...` paths below are relative to
  `/opt/vault` — prefer the absolute form when reading/writing files directly so you
  never mis-resolve the root.
- **Reading local files** (this file, `schema.yaml`, `wiki/HOT.md`, a raw item): use
  **`file_read`** with an absolute path. Do **not** use `read_resource` for vault
  files — it expects a URI and will reject a bare filename.
...
1. **Source page first (dedupe + provenance).** Create `wiki/sources/<YYYY>/<MM>/<slug>.md`
   (`type: source`). Set **`raw:`** to the exact raw path — this is the dedupe key.
