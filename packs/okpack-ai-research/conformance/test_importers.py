...
def test_hf_papers_strict_mode_fetch_and_write():
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        # Always pass --vault: without it the run-report marker targets the real /opt/vault default.
        assert m.main(["--src", "/nonexistent-papers.json", "--vault", t]) == 0
        assert m.main(["--src", "/nonexistent-papers.json", "--vault", t, "--strict"]) == 1
