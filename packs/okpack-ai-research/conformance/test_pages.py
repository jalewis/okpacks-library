...
def test_checker_catches_bad_pages():
    schema = _schema()
    assert check_page({"type": "source", "source_kind": "bogus", "published": "2026-01-01"}, schema)
    assert check_page({"type": "prediction"}, schema)                      # missing base required fields
    assert check_page({"type": "not-a-type"}, schema)                      # unknown type
    # base (engine-owned) types are part of the merged contract, not unknown:
    assert not check_page({"type": "source", "source_kind": "paper",
                           "published": "2026-01-01"}, schema)
    assert check_page({"type": "briefing", "title": "x"}, schema)          # missing `published`
    assert check_page({"type": "prediction", "status": "open", "confidence": 0.5,
                       "subject": "[[x]]", "resolves_by": "2026-12-31",
                       "horizon": "eternal"}, schema)                      # bad horizon enum
    # extensible enum: an unknown modality is allowed (warn-not-fail) -> no error
    assert not check_page({"type": "model", "modality": "proteins"}, schema)
...
if __name__ == "__main__":
    print("== okpack-ai-research page conformance ==")
    n = _run()
