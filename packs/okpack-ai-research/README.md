...
> **Ops.** Pin per-lane models in `<deploy>/.okengine/cron-models.json` (agent lanes default to the
> slowest model). Cron hours run in the gateway TZ (engine default UTC; set `TZ` in `.env`).

## What it builds

- A compounding AI/LLM research wiki for a senior ML practitioner.
- `source` pages for ingested papers, lab posts, and commentary.
- Long-lived `model`, `lab`, `researcher`, `benchmark`, `dataset`, `method`,
  and `concept` pages.
...
- **No active sources yet.** `feeds/feeds.opml` is empty (`feed-fetch` no-ops cleanly
  until you fill it); the suggested AI-research feeds live in `feeds/feeds.opml.example`
  (count + last-probed date in its header comment, kept honest by `validate.py`).
...
pin, validates, and leaves the pack **inert** (nothing deployed or running; the active
feed list is empty, so even once deployed it makes zero upstream calls):
...
The feed-fetch cron in `crons/domain-crons.json` ships enabled (jittered — see
[§Useful by default](#useful-by-default-herd-safe-by-design)); once deployed
(Quickstart step 5) it calls `crons/scripts/okpack_ai_research_feed_fetch.py`,
which wraps the engine's deployed `feed_fetch.py` helper and writes new raw
markdown items under `raw/ai/`. It no-ops cleanly until you populate
`feeds/feeds.opml` (see [§Going live](#going-live-populate-the-feed-list)).
...
| `feeds/feeds.opml.example` | Suggested AI-research sources, grouped by source type; copy entries into `feeds.opml` to opt in. |
...
| `wiki/concepts/` | `method` / `concept` pages — methods, architectures, research themes. |
