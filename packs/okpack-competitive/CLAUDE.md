...
Process each raw item IN ORDER. Read `schema.yaml` for the exact required fields per type.
...
4. **Compile into entities.** Update the `competitor`/`product`/`segment`; append a dated `signal` or
