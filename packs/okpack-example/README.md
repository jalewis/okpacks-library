...
A worked, end-to-end **example** of a complete OKEngine pack — `schema.yaml`, persona `CLAUDE.md`,
feeds, crons, and both co-install forms — that you copy and fill with your own domain. It owns two
throwaway types purely to show the shape; replace everything domain-shaped with yours.

Pinned to engine **okengine v0.10.8** (see `engine.version`).

## What it builds

- **Owns** `topic`, `playbook` — placeholder types that demonstrate the page shape; rename or replace them.
- **Seeded from** nothing by default (the feed list ships empty — add your own sources).
- **Shows** — the full pack skeleton: schema, persona, feeds, crons, and both co-install forms (subdomain + taxonomy).
- **Start here** — copy this directory, rename it, and swap in your domain.

### Safe staged model-write lanes

The starter deliberately separates model-authored ingestion into two stages. `raw-backfill`
compiles each selected raw item into one complete `source` page and emits a verifiable per-item
receipt; it never creates entities or other synthesized pages. `entity-backfill` runs downstream,
consumes accepted source pages, and requires every entity write to cite a source that resolves.

Keep this boundary when customizing the pack. Narrow the example output contract if your source
schema requires more fields, but never widen the raw lane beyond the `sources` namespace. Add
separate contracted lanes for other model-authored page classes instead of combining them into the
raw pass. Partial extraction must be deferred or failed, not committed as an empty canonical.
