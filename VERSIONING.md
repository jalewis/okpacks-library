...
- `engine.version` — the exact required OKEngine release and Hermes runtime pin.

A change to any of these is a **schema-contract change** and requires a version bump in the
same merge request.

An OKEngine or Hermes requirement change is always a pack release, even when the pack schema and
content are unchanged. Compatibility-only restamps use a PATCH bump and receive their own
CHANGELOG section.
